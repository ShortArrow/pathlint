...
/// Outcome status discriminator. Unit-only enum, serialised as a
/// snake_case string under the `kind` field of `pathlint check
/// --json`. Human-readable detail (when present) rides on
/// `Outcome::reason`. See PRD §17 for the wire-shape history.

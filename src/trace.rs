...
impl TraceJsonOutput {
    /// Project a domain `TraceOutcome` into the public JSON shape.
    /// `command` is required because `TraceOutcome::NotFound` does
    /// not carry it but consumers expect to see it echoed back.
    pub fn from_outcome(command: &str, outcome: &TraceOutcome) -> Self {
        match outcome {
            TraceOutcome::NotFound => TraceJsonOutput::NotFound {
                command: command.to_string(),
            },
            TraceOutcome::Found(f) => TraceJsonOutput::Found { inner: f.clone() },
        }
    }
}
...
/// installer name.
#[derive(Debug, PartialEq, Eq, Clone, Serialize, schemars::JsonSchema)]
#[serde(tag = "kind", rename_all = "snake_case")]
pub enum Provenance {

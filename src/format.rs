...
pub fn check_json(outcomes: &[Outcome]) -> Result<String, serde_json::Error> {
    let view: Vec<CheckOutcomeView> = outcomes.iter().map(CheckOutcomeView::from).collect();
    serde_json::to_string_pretty(&view)
}

/// JSON shape emitted by `pathlint check --json`. Each outcome is
/// rendered as one element of a top-level array.
///
/// 0.0.15 renamed the discriminator field from `status` to `kind`
/// so it matches doctor / trace / sort / catalog relations. Schema
/// is generated by `cargo run --bin gen_check_schema` and shipped
/// at `schemas/check.schema.json`.
